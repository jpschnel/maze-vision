...
def main():
    ...
    LoG_img=imread('LoG_image.jpg',cv.CV_LOAD_IMAGE_GRAYSCALE)
    
    cv.rectangle(new_img,top_left_end,bottom_right_end,[0,0,255],-1)
        
    LoG_img_array=np.asarray(LoG_img,dtype="int32")
    
    #print(canny_img_array)
    #for i in range(0,len(LoG_img_array)):
    #    for j in range(0,len(canny_img_array)):
    #        if LoG_img_array[i][j]>120:
    #             LoG_img_array[i][j]=255
    #        else:
    #             LoG_img_array[i][j]=0
    ...
    for i in range(0,len(new_img_array)):
        for j in range(0,len(new_img_array[0])):
            if new_img_array[i][j][0]==0 and new_img_array[i][j][2]==0:
                if LoG_img_array[i][j]==255:
                    new_img_array[i][j][1]=255
                #new_img_array[i][j]=canny_img_array[i][j]
    
    cv.imwrite('extracted.jpg',new_img_array)
   # new_img=imread('extracted.jpg')
